...
1.  Using the `RandomForestRegressor` class and its [scikit-learn documentation page](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html), define a basic model with the following hyperparameters :
    ...
    As it's only a first step drop the `datemut` and `dteloc` columns. 
...
1.  Subsample a fraction ( `0.1` ) of the `df` dataset to reduce computation time and define X_sub and y_sub.
...
X_sub = X_train.sample(frac=0.1, axis=0, random_state= RANDOM_STATE) 
y_sub = y_train[X_sub.index]  # target must be a dataframe with 1 column
...
2.  Train multiple RF models with varying numbers of trees, using `warm_start=True` to avoid retraining all trees from scratch at each iteration. Keep the OOB error calculated in a list.
    ...
    <summary>Click to view a hint</summary>
        ...
        for n in range(min_estimators, max_estimators, 10):
            ...
            if metric == "r2":
                oob_scores.append((n, 1 - rf_pipeline["random_forests"].oob_score_))
            elif metric == "neg_root_mean_squared_error":
                mse = np.mean((y_sub - rf_pipeline["random_forests"].oob_prediction_) ** 2)
                oob_scores.append((n, np.sqrt(mse)))
            else:
                mae = np.mean(np.abs(y_sub - rf_pipeline["random_forests"].oob_prediction_))
