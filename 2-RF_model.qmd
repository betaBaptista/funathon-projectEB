...
::: {.callout-note}
For convenience and pedagogical reasons, we are still working on a subset of the features and with the non transformed target. 
In the real world, we would do this step with the pipeline defined at the end of the [pre-processing part](1-preprocessing.qmd).  
:::

1.  Subsample a fraction ( `0.1` ) of the `df` dataset to reduce computation time and define X_sub and y_sub. To stick to the characteristics of the target, stratify the sampling to the target's distribution. 
...
2.  Train multiple RF models with varying numbers of trees, using `warm_start=True` to avoid retraining all trees from scratch at each iteration. Keep the OOB error calculated in a list.
    ...
    <summary>Click to view a hint</summary>
        ...
        As a hint, you can go to the documentation page on OOB error [here](https://scikit-learn.org/stable/auto_examples/ensemble/plot_ensemble_oob.html) and update the classifier (`clf` in the scikit learn example) to our `rf_pipeline` with what we have seen before to access `oob_score`. 
...
> Note : In the Preprocessing section, you saw how to build a Pipeline. In this exercise, we will use Pipeline again. The first exercise didn't use it to let you practice scikit-learn tools documented [here](https://scikit-learn.org/stable/modules/compose.html#pipelines-and-composite-estimators).
